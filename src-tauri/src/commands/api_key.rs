...
async fn validate_api_key(api_key: &str) -> Result<(), String> {
    ...
    match response.status() {
        StatusCode::UNAUTHORIZED => Err("ElevenLabs APIキーが無効です。".to_string()),
        StatusCode::FORBIDDEN => Err("このAPIキーには必要なアクセス権がありません。".to_string()),
        status => Err(format!(
...
#[tauri::command]
pub(crate) async fn save_api_key(api_key: String) -> Result<(), String> {
