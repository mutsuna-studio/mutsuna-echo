<script lang="ts">
  ...
  let { status, loading, connecting, saving, deleting, busy, onConnect, onCancelConnect, onSelectAccount, onDisconnectOAuth, onSaveManual, onDeleteManual }: Props = $props();
  let apiToken = $state("");
  let accountId = $state("");
