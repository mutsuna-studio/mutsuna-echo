...
Silero VADは独立したダウンロードモデルとして標準導入します。有効時はローカルSTTの前処理で音声区間だけを抽出し、長い連続発話も最大15秒単位に分割して推論します。各区間の前後1秒も認識へ渡し、隣接区間の中間点を保存境界にすることで、VAD境界直前の小さな語頭を保持しながら重複を防ぎます。無音区間を物理的に削除した音声は作らず、各区間の元音声上のオフセットをトークン時刻へ戻すため、Transcriptのタイムスタンプは録音全体の時間軸を維持します。録音中も同じ設定で発話状態だけを表示しますが、録音の自動停止やリアルタイム文字起こしには使用しません。
...
録音、文字起こし、会議ノート生成、LLMによる文字起こし整形の実行中は、端末の自動スリープを抑止します。標準では画面の消灯を許可しますが、設定の「一般」で「処理中は画面を点灯したままにする」を有効にできます。Androidでは長時間処理をフォアグラウンドサービスとして通知し、処理の完了・失敗・キャンセル時にWake Lockを自動解放します。
...
リリース時は3か所のバージョンとストア向け更新文`distribution/whatsnew/whatsnew-ja-JP`を更新してから、同じバージョンのタグをpushします。タグのpushでWindows/macOS成果物をGitHub Releaseの下書きへ追加し、Androidを指定トラックへ配信します。すべて成功後、`release-production`の承認によって下書きが公開されます。Androidの`versionCode`はTauriがSemVerから生成し、`0.1.7`は`1007`になります。
