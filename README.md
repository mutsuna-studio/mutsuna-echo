...
Silero VADも独立した任意モデルとして導入できます。有効時はローカルSTTの前処理で音声区間だけを抽出し、最大30秒単位で推論します。無音区間を物理的に削除した音声は作らず、各区間の元音声上のオフセットをトークン時刻へ戻すため、Transcriptのタイムスタンプは録音全体の時間軸を維持します。VADは録音の自動停止には使用しません。
